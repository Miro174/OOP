...
bool Rus(char ch) {     //проверка на русские буквы

    if (ch >= 0 && ch <= 127) {
        return false;   //если не русские
    }
    return true;        //если русские
}

void printStructur(const std::vector<Structur>& Structur, bool withNumbers) {       //вывод источников 
    for (size_t i = 0; i < Structur.size(); i++) {
        if (withNumbers) {      //если true, то условие выполнено
            std::cout << i + 1 << ". ";
        }
...
int main() {
    ...
    std::ifstream file("bd.txt");   //открытие файла
    if (!file.is_open()) {
    ...
    std::string line;
    while (std::getline(file, line)) {      //работа с текстом файла
    ...
    sort(structur.begin(), structur.end(), [](const Structur& a, const Structur& b) {   //сортировка

        if (Rus(a.author[0]) && !Rus(b.author[0])) {        //пример: (true && !false)
            return true;
        }
        else if (!Rus(a.author[0]) && Rus(b.author[0])) {   //пример: (!true && false)
            return false;
        }
        else {      //остальыне варианты
            return a.author < b.author;
        }
        });

    std::cout << "Пронумерованный список литературы:" << std::endl;
    printStructur(structur, true);      //вывод пронумервоанного списка

    std::cout << std::endl << "Непронумерованный список литературы:" << std::endl;
    printStructur(structur, false);     //вывод не пронумервоанного списка
}